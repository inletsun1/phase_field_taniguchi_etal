...
function main()
    ...
    U = zeros(Float64, YSIZE, XSIZE, TSIZE).+1.0/gamma
    ...
    for t in 2:TSIZE
        ...
        k1u = fu(U[:,:,t-1], V[:,:,t-1], phi[:,:,t-1])
        k1v = fv(U[:,:,t-1], V[:,:,t-1], phi[:,:,t-1])
        k1p = fp(U[:,:,t-1], V[:,:,t-1], phi[:,:,t-1])
        k2u = fu(U[:,:,t-1].+k1u.*dt, V[:,:,t-1].+k1v.*dt, phi[:,:,t-1].+k1p.*dt)
        k2v = fv(U[:,:,t-1].+k1u.*dt, V[:,:,t-1].+k1v.*dt, phi[:,:,t-1].+k1p.*dt)
        k2p = fp(U[:,:,t-1].+k1u.*dt, V[:,:,t-1].+k1v.*dt, phi[:,:,t-1].+k1p.*dt)

        A = sum(phi[:,:,t-1].*dx.*dy)

        abs_nabra_phi = sqrt.(((1.0 ./(2.0.*dx)) .* phi[:,:,t-1] *Bx).^2 .+ ((1.0 ./(2.0.*dy)) .* By *phi[:,:,t-1]).^2)

        del_phi = (eta.*((1.0 ./dy) .^2.0 .* Ay * phi[:,:,t-1] .+ (1.0 ./dx) .^2.0 .* phi[:,:,t-1] * Ax) .-M.*(A .- A0).*abs_nabra_phi .+ (a.*V[:,:,t-1] .- b.*U[:,:,t-1]).*abs_nabra_phi)./tau .+ (k1p .+ k2p)./2.0
        phi[:,:,t] = phi[:,:,t-1] .+ dt.*del_phi

        reaction_U = (k1u.+k2u)./2.0.-Xu.*U[:,:,t-1].*abs_nabra_phi.^2 ./(sum(dx.*dy.*abs_nabra_phi.^2))

        reaction_V = (k1v.+k2v)./2.0.-Xv.*V[:,:,t-1].*abs_nabra_phi.^2 ./(sum(dx.*dy.*abs_nabra_phi.^2))
